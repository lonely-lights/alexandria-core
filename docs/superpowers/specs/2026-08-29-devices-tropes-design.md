...
- **`pattern_marks`** — `id`, `pattern_thread_id` (FK cascade), `role` (`setup` | `develop` | `payoff`), `work_section_id` (FK — the required backbone), `anchor` (nullable jsonb — prose-span refinement in the SAME text-quote format the anchored-comments system uses, reusing its re-anchoring machinery), `note` (nullable text), `created_by`, timestamps.
