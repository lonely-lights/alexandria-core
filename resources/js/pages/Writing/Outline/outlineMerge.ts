...
export function orderOutlineRows(
    ...
    const shapeByKey = new Map(preference.map((r) => [r.key, r]));
    const input = rows.map((r) =>
        parentByKey.has(r.key)
            ? {
                  ...r,
                  ...shapeByKey.get(r.key),
                  parentKey: parentByKey.get(r.key)!,
              }
            : r,
    );
...
export function invalidOutlineParents(rows: OutlineRow[]): string[] {
    ...
    return rows
        .filter((row) => {
            ...
            while (parent !== null) {
                if (!byKey.has(parent) || seen.has(parent)) {
                    return true;
                }
...
export function mergeOutlineDraft(
    ...
    for (const row of [...localRows, ...baseRows, ...serverRows]) {
        if (!keys.has(identity(row))) {
            keys.set(identity(row), row.key);
        }
    }
...
export function resolveOutlineMerge(
    ...
    result.conflicts.forEach((conflict, index) => {
        ...
        if (!choice) {
            throw new Error('Unresolved outline conflict');
        }
