...
export default function BlueprintShow() {
    const { project, blueprint, viewConfig, timelineViewConfig, savedViews, availableColumns, listableBlueprints, relationshipBlueprints, referencingRelationshipBlueprints, projectBlueprints, timelineBlueprints } =
    ...
    const hasTreeView = blueprint.classification === 'structural' || blueprint.show_tree_view;
    ...
    const [viewMode, setViewMode] = useState<ViewMode>(
        blueprint.classification === 'structural' ? 'tree'
        : blueprint.show_tree_view ? 'tree'
        : 'table'
    );
    ...
    useEffect(() => {
        ...
        const activeResolved = resolvedViews.find((v) => v.definition.type === viewMode);
        const activeStillEnabled = activeResolved?.enabledForBlueprint && activeResolved?.applicableForBlueprint;
        ...
        if (!activeResolved) {
            ...
            if (viewMode === 'timeline' && !hasTimelineView) setViewMode('table');
            return;
        }
    ...
    function openSettings(menu?: string) {
        ...
        if (
            ...
            window.dispatchEvent(new CustomEvent('alexandria:open-blueprint-settings', { detail: { menu: menu ?? defaultMenu } }));
        } else {
    ...
    const animation = blueprint.classification === 'standard' ? 'beat-fade' : undefined;
    ...
    return (
        <AppLayout title={`${blueprint.name} - ${project.name}`} immersive>
            <PageHeader
                ...
                actions={
                    <ActionButton
                        ...
                        label={blueprint.classification === 'relationship' ? 'New Connection' : 'New Entry'}
                ...
                tabs={
                    <>
                        ...
                        ) : (hasTreeView || hasTimelineView || hasAnyRegistryView) ? (
                            <>
                                <button
            ...
            <div className="relative overflow-hidden min-h-[min(66vh,calc(100vh-var(--navbar-height,3.5rem)))]">
                ...
                <div className="grid-overlay bg-fade-out pointer-events-none absolute inset-x-0 top-0 h-[66vh] overflow-hidden" aria-hidden="true">
                    <BlueprintGridBg />
                </div>
                ...
                {blueprint.classification === 'relationship' ? (
                    ...
                    <>
                        {visited.has('table') && (
                            <div hidden={viewMode !== 'table' || activeTab !== 'entries'}>
                                <EntriesTab
                                    projectId={project.id}
                                    ...
                                    onRegisterOpenSettings={(fn) => { openSettingsRef.current = fn; }}
                                />
                            </div>
                        )}
                        {hasTreeView && visited.has('tree') && (
                            <div hidden={viewMode !== 'tree' || activeTab !== 'entries'}>
                                <TreeView
                                    projectId={project.id}
                                    blueprint={blueprint}
                                    project={project}
                        ...
                        {hasTimelineView && visited.has('timeline') && (
                            <div hidden={viewMode !== 'timeline' || activeTab !== 'entries'}>
                                <TimelineView
                                    projectId={project.id}
                                    blueprintId={blueprint.id}
                                    availableColumns={availableColumns}
                                    initialConfig={timelineViewConfig ?? undefined}
                                    blueprint={blueprint}
                                    project={project}
                                    listableBlueprints={listableBlueprints}
                                    relationshipBlueprints={relationshipBlueprints}
                        ...
                        {resolvedViews
                            .filter((v) => v.definition.type !== 'tree' && v.definition.type !== 'timeline')
                            .filter((v) => v.enabledForBlueprint && v.applicableForBlueprint && v.accessibleForUser)
                            .map((v) => {
                                const isActive = viewMode === v.definition.type && activeTab === 'entries';
                                const entryConfig = v.entry?.config ?? v.definition.defaultConfig();

                                return (
                                    <div key={v.definition.type} hidden={!isActive}>
                                        {visited.has(v.definition.type as ViewMode) && (
                                            <v.definition.render
                                                blueprint={{
                                                    id: blueprint.id,
                                                    slug: blueprint.slug,
