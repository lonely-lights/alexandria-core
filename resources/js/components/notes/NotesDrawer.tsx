...
import {
    useDrawerHeight,
    type DrawerHeight,
} from "@alexandria/hooks/useDrawerHeight";
import {
    useDrawerMode,
    type DrawerMode,
...
const HEIGHT_OPTIONS: Array<{
...
}> = [
    {
        ...
        labelKey: "notes.drawer.height.partial",
    },
    {
        key: "tall",
        icon: "fa-solid fa-bars",
        labelKey: "notes.drawer.height.tall",
    },
    {
        key: "full",
        icon: "fa-solid fa-up-right-and-down-left-from-center",
        labelKey: "notes.drawer.height.full",
    },
];
    ...
    [
        {
            ...
            labelKey: "notes.drawer.mode.split",
        },
        {
            key: "list",
            icon: "fa-solid fa-list",
            labelKey: "notes.drawer.mode.list",
        },
    ];

const NOTEBOOK_COLOR_SWATCHES = [
    "#6366f1",
    "#ec4899",
    "#f59e0b",
    "#10b981",
...
const headerStyle: CSSProperties = {
    background:
        "linear-gradient(to bottom, color-mix(in srgb, var(--theme-brand-primary-500) 10%, transparent), color-mix(in srgb, var(--theme-brand-primary-500) 5%, transparent))",
    borderBottom:
...
const circleBtn = (
    kind: "import" | "add" | "select-on" | "select-off",
): CSSProperties => {
    ...
    switch (kind) {
        ...
        case "select-on":
            ...
            return {
                ...
                boxShadow:
                    "0 0 0 2px color-mix(in srgb, var(--theme-status-warning-fill) 35%, transparent)",
            };
        case "select-off":
            return {
                ...base,
                background:
                    "color-mix(in srgb, var(--theme-base-content) 18%, transparent)",
                color: "color-mix(in srgb, var(--theme-base-content) 60%, transparent)",
            };
    }
};
...
const notebookSwitchStyle: CSSProperties = {
    background:
        "linear-gradient(90deg, color-mix(in srgb, var(--theme-brand-secondary-500) 20%, transparent), color-mix(in srgb, var(--theme-brand-secondary-500) 10%, transparent))",
    color: "color-mix(in srgb, var(--theme-base-content) 60%, transparent)",
    borderRadius: "var(--theme-radius-button)",
};
...
const tagPillStyle: CSSProperties = {
    background:
        "linear-gradient(90deg, color-mix(in srgb, var(--theme-brand-primary-500) 25%, transparent), color-mix(in srgb, var(--theme-brand-primary-500) 15%, transparent))",
    color: "var(--theme-brand-primary-500)",
...
const tagAddBtnStyle: CSSProperties = {
    ...
    borderRadius: "9999px",
    transition:
        "background var(--theme-motion-duration-fast) var(--theme-motion-easing-standard), color var(--theme-motion-duration-fast) var(--theme-motion-easing-standard)",
};
...
function noteRowStyle(state: "selected" | "checked" | "idle"): CSSProperties {
    const base: CSSProperties = {
        ...
        transition:
            "background-color var(--theme-motion-duration-fast) var(--theme-motion-easing-standard), border-color var(--theme-motion-duration-fast) var(--theme-motion-easing-standard)",
    };
    if (state === "checked") {
        return {
            ...base,
            ...
            boxShadow:
                "0 1px 2px 0 color-mix(in srgb, var(--theme-base-content) 10%, transparent)",
        };
    }
    if (state === "selected") {
        return {
...
const detailBodyStyle: CSSProperties = {
    ...
    background:
        "linear-gradient(to bottom, color-mix(in srgb, var(--theme-base-content) 6%, transparent), color-mix(in srgb, var(--theme-base-content) 3%, transparent))",
    borderRadius: "var(--theme-radius-card)",
    boxShadow:
        "inset 0 2px 4px 0 color-mix(in srgb, var(--theme-base-content) 6%, transparent)",
};

const trashedDetailStyle: CSSProperties = {
    background:
        "color-mix(in srgb, var(--theme-status-error-fill) 15%, transparent)",
    color: "color-mix(in srgb, var(--theme-base-content) 70%, transparent)",
    borderRadius: "var(--theme-radius-card)",
};
...
const headerIconWrapStyle = (
    ...
    return {
        background:
            "color-mix(in srgb, var(--theme-status-error-fill) 25%, transparent)",
...
const portalMenuStyle: CSSProperties = {
    ...
    color: "var(--theme-base-content)",
    boxShadow:
        "0 10px 20px -5px color-mix(in srgb, var(--theme-base-content) 25%, transparent)",
    borderRadius: "var(--theme-radius-card)",
    overflow: "hidden",
};
...
let persistedSwitch: Pick<
    NotesContext,
    "projectId" | "contextType" | "contextId" | "contextLabel" | "contextSlug"
> | null = null;
...
export default function NotesDrawer() {
    ...
    const effectiveDrawerMode: DrawerMode = isMobileDrawer
        ? "list"
        : drawerMode;
    ...
    const [notebookPickerNoteId, setNotebookPickerNoteId] = useState<
        number | null
    >(null);
    const [notebookSelectorOpen, setNotebookSelectorOpen] = useState(false);
    const [notebookLinkAction, setNotebookLinkAction] = useState<{
        notebookId: number;
        action: "link" | "move";
    } | null>(null);
    ...
    const [categorizeWithRelationships, setCategorizeWithRelationships] =
        useState(false);
    ...
    async function batchArchive() {
        ...
        await fetch(
            `/api/v1/projects/${context?.projectId}/notes/batch-archive`,
            {
                method: "POST",
                headers: csrfHeaders(),
                credentials: "same-origin",
                body: JSON.stringify({
                    note_ids: Array.from(checkedIds),
                    action,
                }),
            },
        );
    ...
    /* ── Locations modal ── */
    const [locationsModalNote, setLocationsModalNote] = useState<Note | null>(
        null,
    );
    ...
    /* ── Refs & derived ── */
    const pageAuth = (usePage().props as Record<string, unknown>).auth as {
        user: { id: number };
    } | null;
    ...
    /* Swipe-to-dismiss (mobile): the grab handle above the header tracks a
       downward drag and closes past the threshold. Refs, not state — the
       gesture must not re-render the drawer per touchmove. */
    const swipeStartY = useRef<number | null>(null);
    const swipeLastY = useRef(0);
    ...
    useEffect(() => {
        function handleOpen(e: Event) {
            ...
            if (ctx.preSelectNoteId != null) {
                ...
                persistedSwitch !== null &&
                persistedSwitch.projectId === ctx.projectId
            ) {
                setContext({
                    ...ctx,
            ...
            (data: {
                ...
                setNotes((prev) =>
                    prev.map((n) =>
                        n.id === data.note_id
                            ? {
                                  ...n,
                                  ai_notes: data.ai_notes as Note["ai_notes"],
                              }
                            : n,
                    ),
                );
                ...
                if (data.status === "completed") {
                    ...
                    toast.show(t("notes.drawer.toast.ai_failed.title"), {
                        ...
                        description:
                            data.error ??
                            t("notes.drawer.toast.ai_failed.fallback"),
                    });
                }
            },
        );
        ...
        channel.listen(
            ...
            (data: {
                ...
                if (data.notes_processed > 0) {
                    toast.show(
                        t("ai.workbench.creation.run.toast_done_title"),
                        {
                            type: "success",
                            description: t(
                                "ai.workbench.creation.run.toast_done_desc",
                            )
                                .replace(":notes", String(data.notes_processed))
                                .replace(
                                    ":commands",
                                    String(data.commands_created),
                                ),
                        },
                    );
                } else {
                    toast.show(
                        t("ai.workbench.creation.run.toast_failed_title"),
                        {
                            type: "danger",
                            description:
                                data.error ??
                                t(
                                    "ai.workbench.creation.run.toast_failed_fallback",
                                ),
                        },
                    );
                }
    ...
    useEffect(() => {
        const hasProcessing = notes.some(
            (n) =>
                (n.ai_notes as Record<string, unknown> | null)?.status ===
                "processing",
        );
        ...
        try {
            ...
            if (activeNotebookId) {
                const res = await fetch(
                    `/api/v1/projects/${context.projectId}/notebooks/${activeNotebookId}/notes`,
                    {
                        headers: {
                            Accept: "application/json",
                            "X-Requested-With": "XMLHttpRequest",
                ...
                const res = await fetch(
                    `/api/v1/projects/${context.projectId}/notes?${params}`,
                    {
                        headers: {
                            Accept: "application/json",
                            "X-Requested-With": "XMLHttpRequest",
    ...
    function switchContext(target: SwitchTarget) {
        ...
        if (
            context &&
            target.type === context.contextType &&
            target.id === context.contextId
        ) {
    ...
    function returnToPageContext() {
        ...
        switchContext({
            ...
            id: from.contextId,
            label: from.contextLabel,
            slug: from.contextSlug,
        });
    }
    ...
    async function togglePin() {
        ...
        if (ok) {
            setNotes((prev) =>
                prev.map((n) =>
                    n.id === targetId ? { ...n, is_pinned: nextPinned } : n,
                ),
            );
        }
    }
    ...
    async function generateTitle() {
        ...
        try {
            ...
            if (nextTitle !== null) {
                setNotes((prev) =>
                    prev.map((n) =>
                        n.id === targetId ? { ...n, title: nextTitle } : n,
                    ),
                );
            }
        } finally {
    ...
    async function archiveNote() {
        if (!context || !selectedNote) return;
        await fetch(
            `/api/v1/projects/${context.projectId}/notes/${selectedNote.id}/archive`,
            {
                method: "PUT",
                headers: {
                    Accept: "application/json",
    ...
    async function restoreNote() {
        if (!context || !selectedNote) return;
        await fetch(
            `/api/v1/projects/${context.projectId}/notes/${selectedNote.id}/restore`,
            {
                method: "POST",
                headers: {
                    Accept: "application/json",
    ...
    async function forceDeleteNote(noteId: number) {
        if (!context) return;
        await fetch(
            `/api/v1/projects/${context.projectId}/notes/${noteId}/force`,
            {
                method: "DELETE",
                headers: {
                    Accept: "application/json",
    ...
    async function emptyTrash() {
        ...
        for (const note of notes) {
            await fetch(
                `/api/v1/projects/${context.projectId}/notes/${note.id}/force`,
                {
                    method: "DELETE",
                    headers: {
                        Accept: "application/json",
    ...
    async function createNotebook() {
        ...
        const res = await fetch(
            `/api/v1/projects/${context.projectId}/notebooks`,
            {
                method: "POST",
                headers: {
                    Accept: "application/json",
    ...
    async function addNoteToNotebook(noteId: number, notebookId: number) {
        if (!context) return;
        await fetch(
            `/api/v1/projects/${context.projectId}/notebooks/${notebookId}/notes`,
            {
                method: "POST",
                headers: {
                    Accept: "application/json",
    ...
    async function openTagModal(noteId: number) {
        ...
        const res = await fetch(
            `/api/v1/projects/${context.projectId}/notes/${noteId}/tags`,
            {
                headers: {
                    Accept: "application/json",
                    "X-Requested-With": "XMLHttpRequest",
    ...
    async function fetchPromptPreview() {
        ...
        try {
            const res = await fetch(
                `/api/v1/projects/${context.projectId}/notes/${selectedNote.id}/prompt-preview?context_type=${context.contextType}&context_id=${context.contextId}&with_relationships=${categorizeWithRelationships ? "1" : "0"}`,
                {
                    headers: {
                        Accept: "application/json",
                        "X-Requested-With": "XMLHttpRequest",
    ...
    async function handleCategorize(mode: "send" | "sort") {
        ...
        const contextId =
            mode === "send" ? context.projectId : context.contextId;
        ...
        try {
            if (mode === "sort") {
                ...
                await fetch(
                    `/api/v1/projects/${context.projectId}/notes/batch-categorize`,
                    {
                        method: "POST",
                        headers: csrfHeaders(),
                        credentials: "same-origin",
                ...
                await fetch(
                    `/api/v1/projects/${context.projectId}/notes/${noteIds[0]}/categorize`,
                    {
                        method: "POST",
                        headers: csrfHeaders(),
                        credentials: "same-origin",
            ...
            setNotes((prev) =>
                prev.map((n) =>
                    noteIds.includes(n.id)
                        ? {
                              ...n,
                              ai_notes: { ...n.ai_notes, status: "processing" },
                          }
                        : n,
                ),
            );
            ...
            const autoLabel = categorizeAutoProcess
                ? t("notes.drawer.toast.auto.suffix")
                : "";
            toast.show(`${label}${autoLabel}`, {
                type: "info",
                description: t("notes.drawer.toast.processing.desc").replace(
                    ":count",
                    String(count),
                ),
            });
    ...
    function approveRouting(selectedSlugs: string[]) {
        ...
        void fetch(
            `/api/v1/projects/${context.projectId}/notes/${selectedNote.id}/approve-routing`,
            {
                method: "POST",
                headers: csrfHeaders(),
                credentials: "same-origin",
            ...
            if (res.ok) {
                ...
                toast.show(
                    t("notes.drawer.toast.routed.label").replace(
                        ":count",
                        String(data.blueprint_count),
                    ),
                    {
                        type: "success",
                        description: selectedSlugs
                            .map((s) =>
    ...
    function rejectRouting() {
        if (!context || !selectedNote) return;
        void fetch(
            `/api/v1/projects/${context.projectId}/notes/${selectedNote.id}/reject-routing`,
            {
                method: "POST",
                headers: csrfHeaders(),
                credentials: "same-origin",
    ...
    function reviewCommands() {
        if (!selectedNote) return;
        const batchId = (
            selectedNote.ai_notes as Record<string, unknown> | null
        )?.batch_id;
    ...
    async function triggerEntryIntegration() {
        if (!context || !selectedNote || context.contextType !== "entry")
            return;
        setAiProcessing(true);
        try {
            const res = await fetch(
                `/api/v1/projects/${context.projectId}/notes/${selectedNote.id}/integrate`,
                {
                    method: "POST",
                    headers: {
                        Accept: "application/json",
                    ...
                    credentials: "same-origin",
                    body: JSON.stringify({
                        entry_id: context.contextId,
                        instructions: integrationInstructions || null,
                    }),
                },
            );
            if (res.ok) {
    ...
    return createPortal(
        <>
            ...
            <div
                ref={drawerRef}
                data-notes-drawer
                className={`fixed inset-x-0 bottom-0 z-50 flex flex-col transition-[height] duration-300 ease-out ${isMobileDrawer ? "h-[calc(100dvh-var(--navbar-height,3.5rem))]" : drawerHeightClass}`}
                style={drawerBgStyle}
            >
                {/* Mobile grab handle — 100dvh (not vh: iOS's static unit
                    counts the retracted URL bar, shoving the sheet's top off
                    screen) keeps the header visible, and this bar is the
                    visible swipe-down affordance: drag it past the threshold
                    to dismiss (owner request, phone walkthrough 2026-08-26). */}
                {isMobileDrawer && (
                    <div
                        data-drawer-grab-handle
                        className="flex flex-shrink-0 touch-none justify-center pt-2 pb-1"
                        onTouchStart={(event) => {
                            swipeStartY.current =
                                event.touches[0]?.clientY ?? null;
                            swipeLastY.current = swipeStartY.current ?? 0;
                        }}
                        onTouchMove={(event) => {
                            swipeLastY.current =
                                event.touches[0]?.clientY ?? swipeLastY.current;
                        }}
                        onTouchEnd={() => {
                            if (
                                swipeStartY.current !== null &&
                                swipeLastY.current - swipeStartY.current > 60
                            ) {
                                close();
                            }
                            swipeStartY.current = null;
                ...
                <div
                    className={`flex flex-shrink-0 items-center justify-between ${isMobileDrawer ? "gap-2 px-3 py-2" : "px-4 py-3"}`}
                    style={headerStyle}
                >
                    <h3
                        className={`flex min-w-0 items-center gap-1.5 font-bold ${isMobileDrawer ? "text-base" : "text-xl"}`}
                    >
                        {context && (
                            <button
                                type="button"
                        ...
                        {context &&
                            ...
                            (openedFromRef.current.contextType !==
                                context.contextType ||
                                openedFromRef.current.contextId !==
                                    context.contextId) && (
                                <Tooltip
                                    content={t("notes.switch.back_to_page")}
                                    placement="bottom"
                                >
                                    <button
                                        type="button"
                                        ...
                                        aria-label={t(
                                            "notes.switch.back_to_page",
                                        )}
                                    >
                                        <i
                                            className="fa-solid fa-location-crosshairs text-xs"
                                            aria-hidden="true"
                                        />
                                    </button>
                                </Tooltip>
                    ...
                    <div
                        className={`flex items-center ${isMobileDrawer ? "gap-1" : "gap-2"}`}
                    >
                        {/* Layout mode toggle */}
                        <div
                        ...
                        <Tooltip
                            content={t("notes.drawer.action.sorting_history")}
                            placement="bottom"
                        >
                            <button
                                type="button"
                                ...
                                style={btnSm}
                                aria-label={t(
                                    "notes.drawer.action.sorting_history",
                                )}
                            >
                                <i
                                ...
                                <span className="hidden sm:inline">
                                    {t("notes.drawer.action.sorting_history")}
                                </span>
                            </button>
                        </Tooltip>
                        <Tooltip
                            content={t("notes.drawer.action.dashboard_aria")}
                            placement="bottom"
                        >
                            <a
                                ...
                                style={btnSm}
                                aria-label={t(
                                    "notes.drawer.action.dashboard_aria",
                                )}
                                onClick={close}
                            >
                                <i
                ...
                {effectiveDrawerMode === "list" && context ? (
                    <div
                        className={`min-h-0 flex-grow overflow-y-auto ${isMobileDrawer ? "px-3 py-3 text-[13px]" : "px-4 py-4"}`}
                    >
                        <NotesView
                            projectId={context.projectId}
                        ...
                        >
                            ...
                            <h4 className="mb-4 flex items-center justify-between pl-2 text-lg font-bold">
                                <div className="flex items-center gap-2">
                                    {activeNotebookId ? (
                                        <>
                                            {(() => {
                                                ...
                                                return nb ? (
                                                    ...
                                                    t(
                                                        "notes.drawer.view.notebook_fallback",
                                                    )
                                                );
                                            })()}
                                        </>
                                        ...
                                        <span>
                                            {currentView === "archived"
                                                ? t(
                                                      "notes.drawer.view.archived",
                                                  )
                                                : currentView === "trashed"
                                                  ? t(
                                                        "notes.drawer.view.trashed",
                                                    )
                                                  : t(
                                                        "notes.drawer.view.active",
                                                    )}
                                        </span>
                                    )}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() =>
                                            setNotebookSelectorOpen(true)
                                        }
                                        className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium"
                                        style={notebookSwitchStyle}
                                    >
                                        <i
                                            className="fa-solid fa-book text-[10px]"
                                            aria-hidden="true"
                                        />
                                        {activeNotebookId
                                            ? t(
                                                  "notes.drawer.notebook.button.switch",
                                              )
                                            : t(
                                                  "notes.drawer.notebook.button.browse",
                                              )}
                                        {notebooks.length > 0 && (
                                        ...
                                        {[
                                            {
                                                ...
                                                icon: "fa-folder-open",
                                                tooltipKey:
                                                    "notes.drawer.view_toggle.active_aria",
                                            },
                                            {
                                                key: "archived" as const,
                                                icon: "fa-archive",
                                                tooltipKey:
                                                    "notes.drawer.view_toggle.archived_aria",
                                            },
                                            {
                                                key: "trashed" as const,
                                                icon: "fa-trash",
                                                tooltipKey:
                                                    "notes.drawer.view_toggle.trashed_aria",
                                            },
                            ...
                            <div className="mb-6 mr-2 flex flex-shrink-0 items-center gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        ...
                                        placeholder={t(
                                            "notes.drawer.search_placeholder",
                                        ).replace(
                                            ":count",
                                            String(notes.length),
                                        )}
                                ...
                                <Tooltip
                                    content={t("notes.drawer.tooltip.import")}
                                    placement="bottom"
                                >
                                    <button
                                        type="button"
                                ...
                                <Tooltip
                                    content={t("notes.drawer.tooltip.add")}
                                    placement="bottom"
                                >
                                    <button
                                        type="button"
                                ...
                                <Tooltip
                                    content={
                                        selectMode
                                            ? t(
                                                  "notes.drawer.tooltip.exit_select",
                                              )
                                            : t("notes.drawer.tooltip.select")
                                    }
                                    placement="bottom"
                                >
                                    <button
                                        ...
                                        aria-label={
                                            selectMode
                                                ...
                                                : t(
                                                      "notes.drawer.tooltip.select",
                                                  )
                                        }
                                    >
                                        <i
                                            className={`fa-solid ${selectMode ? "fa-xmark" : "fa-check-double"}`}
                                            aria-hidden="true"
                                        />
                                    </button>
                                </Tooltip>
                                    ...
                                    <Tooltip
                                        content={
                                            checkedIds.size === notes.length
                                                ? t(
                                                      "notes.drawer.tooltip.select_none",
                                                  )
                                                : t(
                                                      "notes.drawer.tooltip.select_all",
                                                  ).replace(
                                                      ":count",
                                                      String(notes.length),
                                                  )
                            ...
                            {selectMode && checkedIds.size > 0 && (
                                <BulkActionsToolbar
                                    ...
                                    onAddToNotebook={() => {
                                        const firstId =
                                            Array.from(checkedIds)[0];
                                        if (firstId)
                                            setNotebookPickerNoteId(firstId);
                                    }}
                                    onLink={() => {
                                        setBulkMoveIds(Array.from(checkedIds));
                                        setLinkAction({
                                    ...
                                    onCopy={() => {
                                        ...
                                        void (async () => {
                                            for (const id of ids) {
                                                await fetch(
                                                    `/api/v1/projects/${context?.projectId}/notes/${id}/link`,
                                                    {
                                                        method: "POST",
                                                        headers: csrfHeaders(),
                                                        credentials:
                                    ...
                                    onTrash={async () => {
                                        for (const id of checkedIds) {
                                            await fetch(
                                                `/api/v1/projects/${context?.projectId}/notes/${id}`,
                                                {
                                                    method: "DELETE",
                                                    headers: csrfHeaders(),
                                                    credentials: "same-origin",
                                        ...
                                        notes.map((note) => {
                                            ...
                                            const rowState:
                                                ...
                                                : selected
                                                  ? "selected"
                                                  : "idle";
                                            const ai = note.ai_notes as Record<
                                                string,
                                                unknown
                                            > | null;
                                            return (
                                                ...
                                                >
                                                    <p className="flex items-center gap-1 truncate font-semibold">
                                                        ...
                                                        {note.is_pinned && (
                                                            <i
                                                                className="fa-solid fa-thumbtack text-[9px]"
                                                                style={{
                                                                    color: "var(--theme-brand-primary-500)",
                                                                }}
                                                                aria-hidden="true"
                                                        ...
                                                        {(() => {
                                                            ...
                                                            // is tough to read").
                                                            const aiDotColor =
                                                                "color-mix(in srgb, var(--theme-status-warning-stroke) 55%, var(--theme-base-content))";
                                                            if (
                                                                typeof ai.routing_count ===
                                                                    "number" &&
                                                                ai.routing_count >
                                                                    0 &&
                                                                !ai.batch_id
                                                                ...
                                                                return (
                                                                    <i
                                                                        ...
                                                                        title={t(
                                                                            "notes.drawer.list.routing_pending_aria",
                                                                        )}
                                                                        aria-label={t(
                                                                            "notes.drawer.list.routing_pending_aria",
                                                                        )}
                                                            ...
                                                            if (ai.batch_id) {
                                                                return (
                                                                    <i
                                                                        ...
                                                                        title={t(
                                                                            "notes.drawer.list.commands_ready_aria",
                                                                        )}
                                                                        aria-label={t(
                                                                            "notes.drawer.list.commands_ready_aria",
                                                                        )}
                                                    ...
                                                    >
                                                        <span>
                                                            {t(
                                                                "notes.drawer.list.byline",
                                                            ).replace(
                                                                ":name",
                                                                note.creator
                                                                    ?.name ??
                                                                    t(
                                                                        "notes.drawer.list.creator_fallback",
                                                                    ),
                                                            )}
                                                        </span>
                                                        <span>
                                                            ...
                                                            note.deleted_at
                                                                ? t(
                                                                      "notes.drawer.list.trashed_label",
                                                                  ).replace(
                                                                      ":date",
                                                                      new Date(
                                                                          note.deleted_at,
                                ...
                                {currentView === "trashed" && selectedNote ? (
                                    <div className="flex h-full flex-col">
                                        <div className="flex flex-shrink-0 items-center justify-between">
                                            <div>
                                                <h4
                                                    ...
                                                    style={{
                                                        color: "color-mix(in srgb, var(--theme-status-error-stroke) 80%, transparent)",
                                                    }}
                                                >
                                                    {selectedNote.title}
                                                </h4>
                                                {selectedNote.deleted_at && (
                                                    ...
                                                    >
                                                        {t(
                                                            "notes.drawer.detail.trashed_on",
                                                        ).replace(
                                                            ":date",
                                                            new Date(
                                                                selectedNote.deleted_at,
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    ...
                                                    />
                                                    {t(
                                                        "notes.drawer.detail.action.restore",
                                                    )}
                                                </button>
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    ...
                                                    />
                                                    {t(
                                                        "notes.drawer.detail.action.delete_forever",
                                                    )}
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        >
                                            <p className="italic">
                                                {t(
                                                    "notes.drawer.detail.trashed_blurb",
                                                )}
                                            </p>
                                            <div style={dividerStyle} />
                                    ...
                                    <div className="flex h-full flex-col">
                                        <div className="flex flex-shrink-0 items-center justify-between">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Tooltip
                                                        content={t(
                                                            "notes.drawer.detail.tooltip.generate_title",
                                                        )}
                                                        placement="bottom"
                                                    >
                                                        <button
                                                ...
                                                >
                                                    {t(
                                                        "notes.drawer.detail.byline_prefix",
                                                    )}
                                                    ...
                                                    {t(
                                                        "notes.drawer.detail.byline_separator",
                                                    )}
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Tooltip
                                                    content={
                                                        selectedNote.is_pinned
                                                            ? t(
                                                                  "notes.drawer.detail.tooltip.unpin",
                                                              )
                                                            : t(
                                                                  "notes.drawer.detail.tooltip.pin",
                                                              )
                                                    }
                                                    placement="bottom"
                                                >
                                                ...
                                                >
                                                    ...
                                                    />
                                                    {t(
                                                        "notes.drawer.detail.action.edit",
                                                    )}
                                                </button>
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    ...
                                                    />
                                                    {t(
                                                        "notes.drawer.detail.action.history",
                                                    )}
                                                </button>
                                                <NoteActionsMenu
                                                    onAddTag={() =>
                                        ...
                                        <div className="mb-3 flex flex-wrap items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                title={t(
                                                    "notes.drawer.detail.action.add_tag",
                                                )}
                                                aria-label={t(
                                                    "notes.drawer.detail.action.add_tag",
                                                )}
                                                    ...
                                                    {selectedNote.links
                                                        ...
                                                        .map((link) => (
                                                            ...
                                                            >
                                                                ...
                                                                <i
                                                                    className="fa-solid fa-arrow-up-right-from-square flex-shrink-0 text-[8px]"
                                                                    style={{
                                    ...
                                    <div className="flex flex-1 items-center justify-center">
                                        <div className="text-center">
                                            <i
                                                className="fa-solid fa-sticky-note mb-3 text-3xl"
                                                style={{
            ...
            <TagPickerModal
                ...
                onToggle={async (tag, selected) => {
                    if (!context) return;
                    const noteIds =
                        bulkTagIds ?? (tagModalNoteId ? [tagModalNoteId] : []);
                    if (selected) {
                        for (const id of noteIds) {
                            await fetch(
                                `/api/v1/projects/${context.projectId}/notes/${id}/tags/${encodeURIComponent(tag)}`,
                                {
                                    method: "DELETE",
                                    headers: csrfHeaders(),
                                    credentials: "same-origin",
                        ...
                        for (const id of noteIds) {
                            await fetch(
                                `/api/v1/projects/${context.projectId}/notes/${id}/tags`,
                                {
                                    method: "POST",
                                    headers: csrfHeaders(),
                                    credentials: "same-origin",
                    ...
                    if (tagModalNoteId) {
                        const res = await fetch(
                            `/api/v1/projects/${context.projectId}/notes/${tagModalNoteId}/tags`,
                            {
                                headers: {
                                    Accept: "application/json",
                                    "X-Requested-With": "XMLHttpRequest",
            ...
            <NotebookSelectorModal
                ...
                onLinkNotebook={(id) =>
                    setNotebookLinkAction({ notebookId: id, action: "link" })
                }
                onMoveNotebook={(id) =>
                    setNotebookLinkAction({ notebookId: id, action: "move" })
                }
            />
            ...
            {context && (
                <ContextSwitchModal
                    ...
                    openedFrom={
                        openedFromRef.current
                            ? {
                                  type: openedFromRef.current.contextType,
                                  id: openedFromRef.current.contextId,
                                  label: openedFromRef.current.contextLabel,
                                  slug: openedFromRef.current.contextSlug,
                              }
                            : {
                                  type: context.contextType,
                                  id: context.contextId,
                                  label: context.contextLabel,
                                  slug: context.contextSlug,
                              }
                    }
            ...
            <CommandReviewModal
                ...
                onExecuted={() => {
                    ...
                    toast.show(t("notes.drawer.toast.executed"), {
                        type: "success",
                    });
                    void fetchNotes();
                }}
                ...
                <div className="p-6">
                    <div className="mb-4 flex items-center gap-3">
                        <div
                            ...
                            style={headerIconWrapStyle("secondary")}
                        >
                            <i
                        ...
                        <div>
                            <h3 className="font-bold">
                                {t("notes.drawer.categorize.title")}
                            </h3>
                            {checkedIds.size > 1 ? (
                                <p className="text-xs" style={fadedText}>
                                    {t(
                                        "notes.drawer.categorize.bulk_count",
                                    ).replace(
                                        ":count",
                    ...
                    <div className="space-y-2">
                        {context?.contextType !== "blueprint" && (
                            ...
                            >
                                <div
                                    ...
                                    style={{
                                        ...headerIconWrapStyle("primary"),
                                        borderRadius:
                                            "var(--theme-radius-input)",
                                    }}
                                >
                                    <i
                                        className="fa-solid fa-share-from-square text-sm"
                                        aria-hidden="true"
                                    />
                                </div>
                                <div>
                                    <p className="text-sm font-semibold">
                                        {t(
                                            "notes.drawer.categorize.send.title",
                                        )}
                                    </p>
                                    <p className="text-xs" style={labelText}>
                                        {t("notes.drawer.categorize.send.desc")}
                                    </p>
                                </div>
                            </button>
                        )}
                        ...
                        {context?.contextType === "blueprint" && (
                            ...
                            >
                                ...
                                >
                                    <i
                                        className="fa-solid fa-layer-group text-sm"
                                        aria-hidden="true"
                                    />
                                </div>
                                <div>
                                    <p className="text-sm font-semibold">
                                        {t(
                                            "notes.drawer.categorize.sort.title",
                                        )}
                                    </p>
                                    <p className="text-xs" style={labelText}>
                                        {t("notes.drawer.categorize.sort.desc")}
                                    </p>
                                </div>
                            </button>
                        )}
                        ...
                        <>
                            <label className="mt-4 flex cursor-pointer items-start gap-3 px-2 py-2">
                                ...
                                <div>
                                    <p className="text-xs font-medium">
                                        {t(
                                            "notes.drawer.categorize.auto.label",
                                        )}
                                    </p>
                                    <p className="text-xs" style={fadedText}>
                                        {t("notes.drawer.categorize.auto.desc")}
                                    </p>
                                </div>
                            </label>
                            ...
                            <label className="flex cursor-pointer items-start gap-3 px-2 py-2">
                                <input
                                    ...
                                    onChange={(e) =>
                                        setCategorizeWithRelationships(
                                            e.target.checked,
                                        )
                                    }
                                />
                                <div>
                                    <p className="text-xs font-medium">
                                        {t(
                                            "notes.drawer.categorize.with_relationships.label",
                                        )}
                                    </p>
                                    <p className="text-xs" style={fadedText}>
                                        {t(
                                            "notes.drawer.categorize.with_relationships.desc",
                                        )}
                                    </p>
                                </div>
                            </label>
            ...
            <Modal
                open={showEntryIntegration}
                onClose={() => setShowEntryIntegration(false)}
                maxWidth="max-w-md"
            >
                <div className="p-6">
                    <div className="mb-4 flex items-center gap-3">
                        <div
                            ...
                            style={headerIconWrapStyle("primary")}
                        >
                            <i
                        ...
                        <div>
                            <h3 className="text-base font-bold">
                                {t("notes.drawer.integration.title")}
                            </h3>
                            <p className="text-xs" style={fadedText}>
                                {t("notes.drawer.integration.subtitle")}
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            onChange={(e) =>
                                setIntegrationInstructions(e.target.value)
                            }
                            placeholder={t(
                                "notes.drawer.integration.instructions_placeholder",
                            )}
                            rows={3}
            ...
            {/* Locations modal */}
            <Modal
                open={!!locationsModalNote}
                onClose={() => setLocationsModalNote(null)}
                maxWidth="max-w-xs"
            >
                <div className="p-5">
                    <h3 className="text-base font-bold">
                        {t("notes.drawer.locations.title")}
                    </h3>
                    <p className="mt-1 text-xs" style={fadedText}>
                        {t("notes.drawer.locations.subtitle_prefix")}
                        {locationsModalNote?.title ||
                            t("notes.drawer.list.untitled")}
            ...
            >
                <div className="p-6">
                    <h3 className="text-lg font-bold">
                        {t("notes.drawer.new_notebook.title")}
                    </h3>
                    <p className="mt-1 text-xs" style={fadedText}>
                        {t("notes.drawer.new_notebook.subtitle")}
                    </p>
                    ...
                    <div className="mt-5 space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder={t(
                                    "notes.drawer.new_notebook.field.title_placeholder",
                                )}
                                autoFocus
                                className="mt-1 w-full text-sm"
                                style={inputStyle}
                        ...
                        <div>
                            ...
                            >
                                {t(
                                    "notes.drawer.new_notebook.field.description",
                                )}
                            </label>
                            <textarea
                                value={newNotebookDesc}
                                onChange={(e) =>
                                    setNewNotebookDesc(e.target.value)
                                }
                                placeholder={t(
                                    "notes.drawer.new_notebook.field.description_placeholder",
                                )}
                                rows={2}
                                className="mt-1 w-full text-sm"
                                style={{ ...inputStyle, resize: "vertical" }}
                            ...
                            <div className="mt-1.5 flex items-center gap-4">
                                <label className="relative cursor-pointer">
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setNewNotebookColor(e.target.value)
                                        }
                                    ...
                                    <div
                                        style={{
                                            ...
                                            borderRadius: "9999px",
                                            boxShadow:
                                                "0 0 0 2px var(--theme-base-100), 0 0 0 4px color-mix(in srgb, var(--theme-base-content) 10%, transparent)",
                                        }}
                                    />
                                </label>
            ...
            <Modal
                open={!!notebookPickerNoteId}
                onClose={() => setNotebookPickerNoteId(null)}
                maxWidth="max-w-sm"
            >
                <div className="p-5">
                    <h3 className="text-base font-bold">
                        {t("notes.drawer.notebook_picker.title")}
                    </h3>
                    <p className="mt-1 text-xs" style={fadedText}>
                        {t("notes.drawer.notebook_picker.subtitle")}
                    </p>
                    {notebooks.length === 0 ? (
                        <div className="mt-4 py-6 text-center">
                            <i
                                className="fa-solid fa-book mb-2 text-xl"
                                style={{
                                    color: "color-mix(in srgb, var(--theme-base-content) 10%, transparent)",
                                }}
                                aria-hidden="true"
                            ...
                            <p className="text-xs" style={microText}>
                                {t("notes.drawer.notebook_picker.empty")}
                            </p>
                        </div>
                    ) : (
                        <div
                            className="mt-3 max-h-48 overflow-y-auto"
                            style={cardHiddenOverflow}
                        >
                ...
                <div className="p-6 text-center">
                    ...
                    <div className="mt-6 flex justify-center gap-3">
                        ...
                        >
                            {confirmAction?.type === "force-delete"
                                ? t(
                                      "notes.drawer.confirm.delete_forever.action",
                                  )
...
function NoteActionsMenu({
    ...
    const items: PortalMenuItem[] = [
        {
            label: t("notes.drawer.actions.add_tag"),
            icon: "fa-solid fa-tag",
            onClick: onAddTag,
        },
        {
            label: t("notes.drawer.actions.add_to_notebook"),
            icon: "fa-solid fa-book",
            onClick: onAddToNotebook,
        },
        {
            label: t("notes.drawer.actions.link"),
            icon: "fa-solid fa-link",
            onClick: onLink,
        },
        {
            label: t("notes.drawer.actions.move"),
            icon: "fa-solid fa-right-from-bracket",
            onClick: onMove,
        },
        {
            label: t("notes.drawer.actions.copy"),
            icon: "fa-solid fa-copy",
            onClick: onCopy,
        },
        {
            label: isArchived
                ? t("notes.drawer.actions.unarchive")
                : t("notes.drawer.actions.archive"),
            icon: "fa-solid fa-archive",
            onClick: onArchive,
        },
        {
            label: t("notes.drawer.actions.trash"),
...
function BulkActionsToolbar({
    ...
    const items: PortalMenuItem[] = [
        {
            label: t("notes.drawer.bulk.categorize"),
    ...
    return (
        <div
            className="mb-2 mr-2 flex items-center justify-between px-3 py-2"
            style={bulkBarStyle}
        >
            <span className="text-xs font-medium">
                {t("notes.drawer.bulk.selected").replace(
                    ":count",
                    String(count),
                )}
            </span>
